...
import { Compass, Mail, Lock, ArrowRight, AlertCircle, Loader2, User } from 'lucide-react';
...
export default function Register() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-805 flex flex-col justify-center items-center p-4 relative">
      ...
      <div className="w-full max-w-md bg-white border border-slate-100 rounded-3xl p-8 sm:p-10 shadow-xl shadow-slate-200/50 relative">
        ...
        <div className="flex flex-col items-center mb-8 text-center">
          ...
          <p className="text-sm text-slate-500 mt-2">Get started with your custom AI Travel Planner</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            <label className="block text-xs font-bold uppercase tracking-wider text-slate-505 mb-2">Confirm Password</label>
            <div className="relative">
              <Lock className="absolute left-3.5 top-1/2 -translate-y-1/2 h-5 w-5 text-slate-400" />
              <input
                type="password"
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-gradient-to-r from-blue-600 to-indigo-650 text-white font-bold py-3.5 px-6 rounded-xl hover:scale-[1.02] active:scale-[0.98] transition-all flex justify-center items-center gap-2 mt-8 disabled:opacity-50 disabled:pointer-events-none"
          >
            {loading ? (
              <>
        ...
        <div className="text-center text-sm text-slate-550 mt-8">
          ...
          <Link to="/login" className="text-indigo-650 hover:text-indigo-500 font-semibold transition">
            Sign In
          </Link>
        </div>
